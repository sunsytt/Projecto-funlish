...
export default function UnidadCard({ nombre, descripcion, numTemas, colorClass, onClick }) {
  return (
    <button onClick={onClick} className="text-left w-full transition-transform duration-200 hover:-translate-y-1">
      <div className="relative">
        ...
        <div className="relative bg-brand-white border border-status-success/30 rounded-2xl p-5 h-56 flex flex-col justify-between shadow-sm hover:shadow-lg transition-shadow">
          <div>
            ...
            {descripcion && (
              <p className="text-brand-midnight/60 text-sm">{descripcion}</p>
            )}
          </div>
